...
var transport = require('../../transport')
  ...
  , ObjectStorageStream = require('./objectStorageStream')
  ;
...
Container.prototype.putStream = function(oname, stream, contentType, callback)
{
  ...
  this.getPublicUrl(function(err, endpoint) {
    ...
    transport.PUT(endpoint, that.authToken, options, function(err, response) {
      if (err) { return callback(err); }
      stream.pipe(new ObjectStorageStream()).pipe(response)
        .on('error', callback)
      ...
      // console.log('************ starting ObjectStorageStream request');
      // callback(null, new ObjectStorageStream(response));
    });

    //   // create a pass-through stream that calculates the MD5 hash
    //   var hashStream = new HashPassThroughStream();
    //
    //   // response is a stream
    //   // var fs = require('fs');
    //   // stream.pipe(hashStream).pipe(fs.createWriteStream('output'));
    //   // callback();
    //   stream.pipe(hashStream).pipe(response);
    //   // response.on('error', callback);
    //   // stream.on('error', callback);
    //   // response.on('response', function(response) {
    //   //   var digest = hashStream.getDigest();
        // if (digest.toLowerCase() !== response.headers.etag.toLowerCase()) {
