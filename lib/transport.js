...
function getHeader(headers, headerToGet) {
  headerToGet = headerToGet.toLowerCase();
  var keys = Object.keys(headers);
  for (var index=0, len=keys.length; index !== len; ++index) {
    var key = keys[index];
    if (key.toLowerCase() === headerToGet) {
      return headers[key];
    }
  }
  return null;
...
function makeRequest(method, uri, authToken, config, callback) {
  ...
  if (authToken && authToken.proxy) {
    options.proxy = authToken.proxy;
